...
//    private static final String getNombreById = "SELECT nombre FROM provincia WHERE idProvincia = ? ;";
    ...
    /*
     * public List<ProvinciaDTO> readByPais(int idPais) { PreparedStatement
     * statement; ResultSet resultSet; ArrayList<ProvinciaDTO> provincias = new
     * ArrayList<ProvinciaDTO>(); Conexion conexion = Conexion.getConexion(); try {
     * statement = conexion.getSQLConexion().prepareStatement(readByPais);
     * statement.setInt(1, idPais); resultSet = statement.executeQuery(); while
     * (resultSet.next()) { provincias.add(getProvinciaDTO(resultSet)); } } catch
     * (SQLException e) { e.printStackTrace(); } return provincias; }
     * 
     * // Unused public String getNombreById(int idProvincia) { PreparedStatement
     * statement; ResultSet resultSet; Conexion conexion = Conexion.getConexion();
     ...
     * nombre = resultSet.getString("Nombre"); return nombre; } catch (SQLException
     * e) { e.printStackTrace(); } return nombre; }
     */
