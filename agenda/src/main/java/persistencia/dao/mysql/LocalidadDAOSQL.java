...
public class LocalidadDAOSQL implements LocalidadDAO {
    ...
    private static final String readByNombreProv = "SELECT * FROM localidad l, provincia p WHERE p.idProvincia "
	    + " = l.provincia AND p.pais=l.pais AND p.nombreProvincia = ?;";
    private static final String readByProv = "SELECT * FROM localidad WHERE Provincia = ?;";
    private static final String readNombreProvinciaById = "select b.nombreProvincia from localidad a inner "
	    + "join provincia b on (a.provincia = b.idProvincia) where a.idLocalidad = ? ;";
    private static final String readIdByNombre = "SELECT idLocalidad FROM localidad WHERE nombreLocalidad = ?;";

    private static final String insert = "INSERT INTO localidad(idLocalidad, nombreLocalidad, Provincia, Pais) VALUES(default, ?, ?, ?);";
    private static final String update = "UPDATE localidad SET nombreLocalidad = ? WHERE idLocalidad = ?;";
    private static final String delete = "DELETE FROM localidad WHERE idLocalidad = ?;";

    private static final String idLocalidadByNombre = "SELECT idLocalidad FROM localidad WHERE nombreLocalidad = ?;";
	...
	try {
	    ...
	    resultSet = statement.executeQuery();
	    nombreProvincia = resultSet.getString("NombreProvincia");
	} catch (SQLException e) {
	    e.printStackTrace();
	}
	...
	try {
	    ...
	    statement.setString(1, nuevaLocalidad.getNombreLocalidad());
	...
	try {
	    ...
	    statement.setString(1, editLocalidad.getNombreLocalidad());
	...
	List<LocalidadDTO> localidad = new ArrayList<LocalidadDTO>();
	Conexion conexion = Conexion.getConexion();
	try {
