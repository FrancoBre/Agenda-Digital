...
public class ControladorAgregar implements ActionListener
{
		...
		public ControladorAgregar(Vista vista, Agenda agenda)
		{
			...
			this.ventanaPersonaAgregar.getProvinciaInput().addActionListener(comboLocalidad -> cambioItemsLocalidad(comboLocalidad));			
			addComboboxItems();
		}
		...
		private void guardarPersona(ActionEvent p) {
			if (!ventanaPersonaAgregar.validarRequeridos()) {
				...
				try {
					nuevaPersona = new PersonaDTO(0, nombre, tel, email, Fecha.parseNacimiento(nacimiento),
		...
		private void cambioItemsProvincia(ActionEvent c) {
			...
			String nombrePais = (String) this.ventanaPersonaAgregar.getPaisInput().getSelectedItem();
